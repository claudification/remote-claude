	...
	return (
		<div className="overflow-y-auto h-full">
			...
			<div className="sticky top-0 bg-background/95 backdrop-blur border-b border-border px-3 py-2 flex gap-4 text-[10px] font-mono">
				...
				{inProgress.length > 0 && (
					<span className="text-active">{inProgress.length} active</span>
				)}
			...
			{groups.map(group => (
				<div key={group.label} className="mb-2">
					...
					{group.tasks.map(task => (
						<div
							key={task.id}
							className="px-3 py-2 border-b border-border/50 hover:bg-muted/20 transition-colors"
						>
							...
							{task.owner && (
								<div className="mt-1 ml-6 text-[10px] text-muted-foreground font-mono">
									owner: {task.owner}
								</div>
							)}
						</div>
