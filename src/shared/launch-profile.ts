...
export const LAUNCH_PROFILE_MAX_NAME = 64
export const LAUNCH_PROFILE_MAX_SHORT_LABEL = 24
...
export type BackendWithAppendSystemPrompt = (typeof BACKENDS_WITH_APPEND_SYSTEM_PROMPT)[number]
...
export const PROFILE_COLOR_OPTIONS = ['primary', 'success', 'warning', 'destructive', 'info', 'muted'] as const
...
const profileSpawnSchema = spawnRequestSchema
  ...
  .extend({
    appendSystemPrompt: z
      .string()
      .max(LAUNCH_PROFILE_MAX_APPEND_SP, 'appendSystemPrompt exceeds 16 KB cap')
      .optional(),
  })
  .partial()
...
export const launchProfileSchema = z.object({
  ...
  name: z.string().min(1, 'name is required').max(LAUNCH_PROFILE_MAX_NAME),
  shortLabel: z.string().max(LAUNCH_PROFILE_MAX_SHORT_LABEL).optional(),
...
export function isLaunchProfileId(id: unknown): id is string {
  return typeof id === 'string' && id.startsWith(LAUNCH_PROFILE_ID_PREFIX) && id.length > LAUNCH_PROFILE_ID_PREFIX.length
}
