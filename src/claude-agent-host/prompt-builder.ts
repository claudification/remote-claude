  ...
  return [
    ...
    ...(channelEnabled
      ? [
          ...
          '- `mcp__rclaude__list_conversations` - discover live sessions (only shows channel-capable sessions)',
          '- `mcp__rclaude__send_message` - send a message to another session (first contact requires user approval via dashboard)',
          '',
          '## Session Addressing',
          '',
          'Each session has a stable ID returned by `list_conversations`. When a project directory has a single session,',
          ...
          '  it is guaranteed to match what `list_conversations` would return for that session (bare or compound).',
