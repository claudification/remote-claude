...
export function createSchema(db: Database) {
  ...
  // FTS5 over recap content. No triggers -- explicit sync from application
  // code on markRecapDone (per plan-recap.md decision: triggers can't easily
  // join JSON arrays from metadata_json, so we do it in JS).
  db.run(`
