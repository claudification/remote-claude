  ...
  mcp.tool(
    ...
    async ({ to, message, intent }) => {
      ...
      const results = targets.map(t => {
        ...
        return { to: t, ok: true, status: 'delivered' as const, targetSessionId: target.id }
      })

      if (!isArrayTarget) {
        const r = results[0]
        if (!r.ok) return { content: [{ type: 'text', text: `Target "${r.to}" not found or not connected` }] }
        return { content: [{ type: 'text', text: `Message sent to ${r.targetSessionId}` }] }
      }
      const delivered = results.filter(r => r.ok).length
      const failed = results.length - delivered
      const lines = [`Multicast to ${results.length} target(s): ${delivered} delivered, ${failed} failed.`]
      for (const r of results) {
        const detail = r.ok ? `(target_session_id: ${r.targetSessionId})` : `-- ${r.error}`
