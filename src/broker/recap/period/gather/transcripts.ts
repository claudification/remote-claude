...
export function gatherTranscripts(
  ...
  return conversations.map(conv => ({
    ...
    turns: digestForConversation(store, conv.id, scope),
  }))
}

function digestForConversation(store: StoreDriver, id: string, scope: PeriodScope): PeriodTurn[] {
  ...
  return extractUserPromptsAndFinals(entries.map(toLooseTranscriptEntry))
}
